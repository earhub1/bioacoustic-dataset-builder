...
7. **(Opcional) Visualizar sequências**: use `python src/visualize_sequence_colormesh.py` para gerar um colormesh simples das fitas (freq x frames) a partir do `manifest_sequences.csv`. Você pode filtrar por `sequence_idx`, `segment_idx` e split, e limitar a janela com `--max-plot-duration` (10 fps por padrão com `hop_length=6400` e `target_sr=64000`). Consulte `docs/visualize_sequences.md` para exemplos.
